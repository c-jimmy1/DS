...
template <class T> unsigned int remove_matching_elements(Vec<T>& v, const T& element) {
    typename Vec<T>::iterator v_iterator = v.begin();
    unsigned int count = 0;
    while (v_iterator != v.end()) {
        if (*v_iterator == element) {
            v_iterator = v.erase(v_iterator);
            count += 1;
        }
        else {
          v_iterator++;
...
template <class T> void Vec<T>::print() const {
  std::cout << "Allocated Memory: " << m_alloc << std::endl;
