...
private:
  ...
  TreeNode<T>* copy_tree(TreeNode<T>* old_root) {
    ...
    // base case: if old root is null, return null
    if (old_root == nullptr) {
      return nullptr;
    }
    // create a new node with the same data as old root
  ...
  // loop until p is null or p's value matches the key value
  while (p != nullptr && p->value != key_value) {
    // if key value is less than p's value, go to left child
    if (key_value < p->value) {
      p = p->left;
    }
    // if key value is greater than p's value, go to right child
    else {
      p = p->right;
    }
  }
  // return an iterator pointing to p (either null or matching node)
  return iterator(p);
  }
