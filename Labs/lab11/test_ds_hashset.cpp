...
// Default (optimal) hash function, minimal collisions
class BestHashFunction
{
public:
...
// bad hash function, tons of collisions
class TerribleHashFunction
{
public:
    unsigned int operator() ( std::string const& key ) const
    {
...
// collisions for words with the same letters in any order
class AnagramHashFunction
{
public:
    unsigned int operator() ( std::string const& key ) const
    {
...
typedef ds_hashset<std::string, BestHashFunction> ds_hashset_type;
...
int main()
{
    ...
    insert_result = set1.insert( std::string("hello") );
