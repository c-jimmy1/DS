...
public:
  ...
  Balloon* BalloonPoint() {return this;}
...
void deleteAll(const Balloon* root, std::set<const Balloon*>& nodes)
{
    typename std::set<const Balloon*>::const_iterator itr = nodes.find(root);
    if (itr != nodes.end()) {return;}
    int rope_num = root->numRopes();
    nodes.insert(root);
    for (int i = 0; i < rope_num; ++i)
    {
      deleteAll(root->getRope(i), nodes);
    }
}

void deleteAll(Balloon* root)
{
    std::set<const Balloon*> nodes;
    deleteAll(root, nodes);
    for (typename std::set<const Balloon*>::const_iterator itr = nodes.begin(); itr != nodes.end(); ++itr)
    {
      delete *itr;
    }
}
...
int main() {
  ...
  */
  ...
  dsSharedPtr<Balloon> snoop(henry);
  georgette->addRope(henry->BalloonPoint());
  henry = new Balloon("Tigger");
  georgette->addRope(henry->BalloonPoint());
  georgette->print();
  henry->print();
  ...
  henry->addRope(isabelle->BalloonPoint());
  dsSharedPtr<Balloon> shrek(isabelle);
  isabelle = new Balloon("Barney the Purple Dinosaur");
  georgette->addRope(isabelle->BalloonPoint());
