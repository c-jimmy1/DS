...
std::vector<std::vector<Point> > validPaths;
...
void print_grid(const std::vector<std::vector<GRID_STATUS> > & blocked_grid,
    ...
    for (int c = 0; c < validPaths.size(); c++) {
        std::cout << "Valid Path #" << c << ": ";
        for (int d = 0; d < validPaths[c].size(); d++) {
            std::cout << "(" << validPaths[c][d].x << ", " << validPaths[c][d].y << ") ";
        }
        std::cout << std::endl;
    }

    if (validPaths.size() == 0)
        std::cout << "No valid paths";
}

int calculatePaths(int x, int y, std::vector<std::vector<GRID_STATUS> >& blocked, std::vector<Point> path)
{
    if ((x == 0 && y == 0)) {
        path.push_back(Point(x, y));
        validPaths.push_back(path);
        for(int i=0;i<path.size();i++){
            blocked[path[i].x][path[i].y]=PATH;
        }
    ...
    if (x == 0 && y > 0)
        return calculatePaths(x, y-1, blocked, path);
    else if (y == 0 && x > 0)
        return calculatePaths(x-1, y, blocked, path);
    return calculatePaths(x-1, y, blocked, path) + calculatePaths(x, y-1, blocked, path);
}
...
int main(int argc, char* argv[]) {
    ...
    std::cout << calculatePaths(start_x, start_y, blocked_grid, path) << " paths" << std::endl;
