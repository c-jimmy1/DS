...
void StudentTest(){
	...
	// TESTING IF GET_COL GETS THE CORRECT VALUES
	double* c1 = NULL;
	c1 = m1.get_col(2);
	assert(c1[0] == 3 && c1[1] == 6);
	delete [] c1;

	// TESTING TRANSPOSE METHOD AND PRINTING OUT
	m1.transpose();
	m2.transpose();
	std::cout << "\nTranspose for M1" << std::endl;
	std::cout << m1 << std::endl;
	std::cout << "\nTranspose for M2" << std::endl;
    std::cout << m2 << std::endl;

	// TESTING SUBTRACT METHOD BY SUBTRACTING M1 BY M3
	m1.subtract(m3);
	std::cout << "\n Subtracting M1 by M3" << std::endl;
	std::cout << m1 << std::endl;

	// TESTING SWAP_ROW WITH DATA UNCHAGED FROM SWAP ROWS
	m2.swap_row(0,1);
	std::cout << "\nSwap row for M2" << std::endl;
	std::cout << m2 << std::endl;

	// TESTING QUARTER AGAIN WITH DATA UNCHANGED FROM TRANSPOSE AND SWAP ROWS
	Matrix* ma1 = NULL;
	ma1 = m2.quarter();
	assert(ma1 != NULL);

	std::cout << "M2: UL: " << std::endl << ma1[0] << std::endl;
	std::cout << "M2: UR: " << std::endl << ma1[1] << std::endl;
	std::cout << "M2: LL: " << std::endl << ma1[2] << std::endl;
	std::cout << "M2: LR: " << std::endl << ma1[3] << std::endl;
